...
use app\home\model\Guoyanping as GuoyanpingModel;
...
// 进行分页数据查询 注意page方法的参数的前面部分是当前的页数使用 $_GET[p]获取
    ...
    public function add()
    {
        ...
        if (GuoyanpingModel::add($data)) {
            $this->sendMsg($data['name'], $data['phone']);
        } else {
            $error = GuoyanpingModel::getError();
    ...
    public function sendMsg($name, $phone)
    {
        ...
        $request->setPhoneNumbers("13862428284");
        //必填-短信签名
        ...
        $this->success("您的贷款申请已提交成功，客户经理会尽快与您联系，请保持电话畅通！", url("Guoyanping/index"));
    }
